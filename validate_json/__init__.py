"""
    validateJSON: validate JSON data against a schema
"""
# logging
import logging
...
# config obj
from validate_json.config import LOGGING_DICT as logging_cfg
...
class JSONValidator:
    ...
    def validate_expected(self, json_object, expected_keys):
        ...
        if not all(contains_params):
            ...
            missing_keys = [
                ...
                for expected_key_name, exists in zip(expected_keys, contains_params)
        ...
        if not all(correct_types):
            ...
            incorrect_type_keys = [
                # get param name
                expected_key_name["param_name"]
                for expected_key_name, correct_type in zip(expected_keys, correct_types)
        ...
        for expected_key in expected_keys:
            ...
            if expected_key["param_type"] == dict:
                ...
                if "conditional_keys" in expected_key:
                    ...
                    depends_on_val = json_object[
                        expected_key["conditional_keys"]["depends_on"]
                    ]
                    # get depdendence info (i.e. based on the above value
                    # get what the expected / optional keys are)
                    nested_optional_keys = expected_key["conditional_keys"][
                        "dependence_info"
                    ][depends_on_val]["optional_keys"]
                    nested_expected_keys = expected_key["conditional_keys"][
                        "dependence_info"
                    ][depends_on_val]["expected_keys"]
    ...
    def contains_invalid_keys(
        self, json_object, valid_keys
        ...
        contains_only_valid = [
            ...
            parameter in valid_keys
            for parameter in json_object
        ]
        ...
        if not all(contains_only_valid):
            ...
            invalid_keys = [
                # name of key
                invalid_key
                for invalid_key, is_valid in zip(
                    list(json_object.keys()), contains_only_valid
                )
    ...
    def build_valid_keys(
        self, expected_keys=None, optional_keys=None
    ):  # pylint: disable=no-self-use
        """
    ...
    def validate(self, json_object, expected_keys=None, optional_keys=None):
        ...
        # check if any parameter present in JSON object is not in valid_keys (i.e. check
        # it is allowed)
        if self.contains_invalid_keys(json_object=json_object, valid_keys=valid_keys):
            # return
            return False
        ...
        if expected_keys is not None:
            ...
            if optional_keys is not None:
                # run validation
                return self.validate_optional(
                    json_object, optional_keys
                ) and self.validate_expected(json_object, expected_keys)
            # run validation
